        ...
        <FormItem :label="$t('audio-bit-rate')"
          ...
          <div class="bit_rate_range media_cut_item"
            ...
            <Slider style="flex: 1;"
                    :min="allBitRates[0]"
                    :max="allBitRates[1]"
                    @on-input="value => formData.bitRate=value * 1000"></Slider>
            <span v-if="formData.bitRate != '-1'">{{formData.bitRate !== '0' ? parseFloat(formData.bitRate) /1000 : allBitRates[2]}} kbps</span>
            <span v-else
